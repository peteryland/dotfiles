...
if [[ -z $(find volcanos.marker -mmin -60 2> /dev/null) ]]; then
  if data="$(wget https://volcano.si.edu/news/WeeklyVolcanoRSS.xml -qO -)"; then
    grep -E ^\<title\|georss:point <<< "$data" | cut -d\> -f2 | cut -d\< -f1 | while read x; do
      if [[ $title ]]; then
        coords="$x"
    ...
    touch volcanos.marker
  fi
fi

if [[ -z $(find quakes.marker -mmin -60 2> /dev/null) ]]; then
  if data="$(wget https://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/4.5_week.geojson -qO -)"; then
    jq -r '.features[] | {properties, geometry}
                       ...
                       | "\($coords) \"\" \($color) symbolsize=0 \($size) \($opacity) align=center\n" +
                         "\($coords) \" \(if $days <= 1 and .properties.mag >= 6 then .properties.title else .properties.mag end)  \" \($color) symbolsize=0 \($size) \($opacity)"' <<< "$data" > quakes.marker
  else
    touch quakes.marker
  fi
fi
